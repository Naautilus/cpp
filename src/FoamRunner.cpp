...
const double ATMOSPHERE_TO_EXIT_RADIUS_RATIO = 2.5; // cells between the chamber and atmosphere regions need to have matching faces which only integers and specific ratios work for
...
int main() {
  ...
  structureData.insert(structureData.end(), structureLayer(ATMOSPHERE_REGION_RADIUS, ATMOSPHERE_REGION_RADIUS*0.5, ATMOSPHERE_REGION_RADIUS, ATMOSPHERE, 0.3, ATMOSPHERE_TO_EXIT_RADIUS_RATIO, 1));
  int angularResolution = 8;
  vector<double> cellResolutionsPerUnit = {1, 3, 4}; // {side-to-side radius-wise, vertical, in-out}
